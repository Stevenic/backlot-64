...
- Per-frame code is engine assembly in `src/` and in the engine's modules in `modules/` (physics, collision, AI, the cutscene), which the engine pages in from the REU. Games supply data and scripts, never per-frame code.
