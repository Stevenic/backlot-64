...
4. **DMA is the blitter and the loader.** The REU copies a byte in every cycle the VIC does not take, with the CPU halted: a kilobyte in about 1,130 cycles in the border and 1,210 inside the display, setup included (measured, `budgets.txt`). Screen shifts, colour RAM shifts, charset swaps, palette swaps, and code overlays are DMA jobs, not CPU loops.
...
| Storage | REU of at least 8 MB (VICE, Ultimate 64, C64 Ultimate, 1541 Ultimate II+; a real 1750 is 512 KB and is not enough) | Same REU; the C128 supports it natively |
| Input | Joystick port 2, keyboard | Same |
| Emulator | `x64sc` | `x128` |

**C64 frame budget** on PAL: 312 lines, 63 cycles per line, 19,656 cycles per frame, less badline stalls of 40 to 43 cycles on 25 lines of the visible area, giving about 18,600 usable cycles before sprites. Each displayed sprite takes a further 2 cycles on every raster line it covers, plus up to 3 cycles of lead-in, so a row of eight sprites costs about 400 cycles more. (Bauer, sections 3.5 and 2.4.3.)

**C128 frame budget:** the same during the badline range (raster $30 to $F7), because the VIC-IIe cannot display in 2 MHz mode. The engine switches to 2 MHz after the last badline and back before line $30. That is 7,056 border cycles at 1 MHz becoming 14,112, so about 25,600 usable cycles per frame, 38 percent more than the C64. The gain is CPU only. REU DMA runs at 1 MHz on both machines (the engine issues every DMA in slow mode; whether the REC tolerates fast mode is not established), so the scroller's DMA blits cost the same and the extra time goes to modules, scripts, and audio.
...
**The VDC as a development console.** The 80-column chip is too slow to write for gameplay and needs an RGBI-capable monitor or input on real hardware, so the engine does not use it for the player. Debug builds put a live console on it: the frame's cycle count per hook, the entity table, the running script threads, the last twenty events, and the benchmark harness output. `x128` shows the VDC in its own window, which makes it the best debugger the project will have. Release builds leave the VDC blank.

**Trading memory for cycles.** Bank 1 is also how the C128 runs faster per-entity modules, not only more of them. On a 6502, memory buys speed through lookup tables: 2 KB of quarter-square tables turn an 8-bit multiply from a shift-and-add loop of over a hundred cycles into about 30 to 60 (Codebase64, "Seriously fast multiplication"), a sine table replaces rotation math, a per-sector distance field turns pathfinding into a table read, a line-of-sight table replaces a tile ray. The C64 has no room for these. The C128 has 60 KB of bank 1 for them.

The mechanism: the MMU's common area is set to the low 16 KB, so the engine core and the per-entity modules at $0800 to $3FFF are visible in both banks. Hot code selects bank 1 with one store to a load-configuration register ($FF01 to $FF04, preloaded through $D501 to $D504), reads its table, and selects bank 0 with another: 4 cycles each way, 8 per lookup, against savings of 50 to 120. The VIC keeps reading bank 0 throughout, because its bank select is a separate MMU register ($D506 bits 6 and 7). Those same bits steer REU DMA, so a DMA reaches bank 1 only while the VIC is pointed there too: bank 1 is filled at init with the screen blanked, and DMA targets during play stay in bank 0 or common RAM. ("Mapping the Commodore 128", MMU and REC chapters.)
...
| **Worst case** | **~6,000** | About a third of the frame (the target; see the measurement below) |
...
The world is a flat 2048x2048 array of metatile indices in the REU, 4 MB. Address is `(y << 11) | x`, so a metatile lookup is three register writes and one DMA. A metatile is 4x4 cells, 32x32 pixels. At Priors-64's scale of roughly 2.5 metres per cell, a metatile is 10 metres and the world is 20.5 km on a side, about 420 square kilometres. The largest open-world city maps fit inside that with room to spare.
...
A full tileset switch, for interiors or a different world, is a DMA of 10 KB, about 11,000 cycles with the display on, spread over two frames behind a raster-timed colour ramp so it never shows. Switching day and night within a tileset is a 4 KB DMA plus register writes.

### 3.4 Sprites

Eight hardware sprites, seven of them multiplexed (hardware sprite 0 is pinned for the object that must never flicker, the player) or all eight when a game unpins sprite 0. The number of virtual sprites a list may hold follows the CPU speed (`spr_limit`, set by `b64_turbo_set`): 24 on a stock C64, 32 with the Ultimate's turbo at 2 to 12 MHz, 64 at 16 MHz and up. Every decision is made in the main loop by `b64_spr_end`: the sort order is kept from frame to frame and re-sorted by y; each entry takes the hardware sprite freed longest ago; an entry the chain cannot write in time is dropped whole, so an overloaded line loses sprites but never shows one cut short or late; and consecutive entries share one raster interrupt while all of them can still be written before the first is due. The interrupts only copy: each entry is one index byte, and a group runs through routines generated at start, one per hardware sprite with its registers built in, each chaining to the next. A chain interrupt that finds the next group already due runs it at once. (Roadmap step 9, 2026-09-18; the design and its measurements are in `docs/JOURNAL.md`.)

**The chain's timing model.** Whether an entry can be written in time is decided from three figures in raster lines, set with the speed: the lead from a group's interrupt to its first entry written, plus a line to spare (`mux_lead1`: 5 at 1 MHz, 3 at 2 to 12 MHz, 2 above); the lines each further entry in a group adds (`mux_step`: 2 at 1 MHz, where eight sprites' DMA leaves about 44 cycles a line, 1 with the turbo); and the chain itself, which cannot start a group until it has finished the one before, so a group starts at its line or at the model's end of the previous group, whichever is later. The last is what keeps a dense layout honest: at 1 MHz a lone group takes 3 to 4 lines with all eight sprites on the line, so groups every 3 lines fall further behind with each one, and without the model the 64-sprite harness wrote about 40 entries a frame late. With it, the same layout at 1 MHz shows 51 of 64 and writes none late; the cost is about 45 cycles a sprite in `b64_spr_end`. The turbo figures are UNTESTED: `tools/b64muxhw.py` measures them on the hardware (`docs/ULTIMATE.md`).

**Traffic keeps within the lines.** The VIC-II shows at most eight sprites on a raster line at any speed, and the player's car, on the pinned sprite 0, is always one of them, so everything else may put at most seven on a line. `examples/traffic` shows how a game keeps to that without the multiplexer ever having to drop a car: world lines are counted in bands of 8, and every live car, on screen or in the margin around it, holds the bands its sprite covers plus the 17 lines below it that the multiplexer needs before it can reuse that hardware sprite (a 5-line lead and 2 lines a sprite for a row of seven, at 1 MHz). A car does not spawn into a full band, and a car driving up or down the screen waits before it enters one; cars driving east or west never change bands. So an east-west street carries at most seven moving cars besides the player, and a north-south street is limited only by its length. Cars keep their lanes from the map's road bits (`PREPARE.md`, tilesets), turn only right or go straight, stop at lights on a 512-frame cycle with 64 frames of all-red, queue behind the car ahead, stay out of a crossing whose exit is jammed, and look ahead with their real sizes (24 x 6 driving east or west, 12 x 15 north or south), so a car crossing one's path is seen. Measured driving itself on the stock tier for 3,000 frames: no car overlaps another, no line holds more than seven, the multiplexer drops nothing, 12 to 14 cars live and 10 to 12 on screen, and 1 to 5 percent of frames lost depending on the route (`traffic.*` in `make check`). The first cut of the path/road module and the entity system's budget (roadmap step 14).

**Frames are streamed per slot, not loaded in banks.** Each of the 48 sprite slots owns 64 bytes of VIC RAM. A game submits a sprite with its slot number and the 24-bit REU address of the frame it wants; the engine compares that address with what the slot holds and DMAs the 64 bytes only when it differs. An entity keeps the same slot from frame to frame, so a car driving straight costs nothing and a car turning costs one 100-cycle DMA. The resident sprite memory is 3 KB, and the number of distinct sprite designs that can be on screen at once is simply the number of slots; a crowd of 64 shares them, several entries showing the same frame. This is what lets a game have 150 vehicle types in traffic with no bank juggling. A worst case where every visible sprite changes frame in the same frame is 24 DMAs, about 2,400 cycles, and does not happen in practice.
...
Budget, measured in the harness (`examples/mux`, `make check`) at its densest layout, a sprite every 4 lines: 32 sprites cost 8,586 cycles in `b64_spr_end` (interrupts off; 7,462 before the chain model) plus about 3,500 in `b64_spr_add`, and 8,389 cycles of interrupts a frame; the whole list build with its interrupts takes about the entire frame. It holds 50 Hz without a damaged sprite on a stock C64, but leaves nothing for a game, so the list limit is 24 on a stock machine (`spr_limit`, which a game may raise itself). The previous multiplexer cost 8,172 and 8,891 at 24. Overloaded, 12 sprites bunched into 24 lines, it shows the seven a line can hold and drops the rest whole; the previous one drew some damaged. The 64-sprite tier has its own harness (`examples/mux` assembled with `MUX64`: a sprite every 3 lines down 192 lines, all eight hardware sprites) and its own test build with every entry time-stamped (`build/mlog/`, `docs/INSTRUMENT.md`); at 1 MHz a 64-sprite list takes more than a frame to build, which is why the tier needs the turbo.
...
A cutscene is layers inside raster bands, because the VIC-II has one graphics mode at a time, so the engine changes it at raster splits.

| Layer | What | Mechanism | Cost |
|---|---|---|---|
| 0 | The set | A 160x200 multicolour bitmap still (double-wide pixels filling the 320x200 window) streamed from the REU; the original stays in the REU | 10 KB DMA behind a blank |
...
**Fetch and stash.** `b64_fetch` and `b64_stash` move up to 64 KB between a 24-bit REU address and a 16-bit C64 address in one DMA. Cost is one cycle per byte the VIC does not steal, about 1.1 with the display on, plus about 105 cycles of setup in the engine's call (measured; the REU itself has no setup cost). The REU registers are written directly. There is no queue, no interrupt, no asynchrony: DMA halts the CPU, so when the call returns the bytes are there.
...
|---|---|---|---|
| **physics** | **arcade** 1.0 KB, 50 + 60/ent. 8.8 fixed-point velocity, per-vehicle turn rate, tile collision with stop. Top-down arcade feel. | **grip** 1.6 KB, 80 + 110/ent. Adds drag, skid above a grip threshold, surface friction from tile properties, handbrake turns. | **sim** 2.5 KB, 100 + 180/ent. Mass, momentum transfer between vehicles, damage from impulse, trailers and towing. |
...
**Frame cycles.** The rendering worst case is about 10,400 cycles today (6,000 is the target). The remainder of the 18,600 is for modules and the game. The packer computes `fixed + per_entity x active_entities` for the selected modules and reports the total as a percentage of the frame at the entity count the manifest declares. Over 100 percent is an error. Over 80 percent is a warning, because worst cases stack.
...
Scripts decide, hooks execute. Nothing that runs per entity per frame is ever bytecode, because a VM opcode costs 82 to 118 cycles on this engine (measured), most of it dispatch and operand fetch, and a scripted loop over 32 entities calling three modules each would spend the whole frame on it. Equally, nothing that is a game rule is hard-wired into a hook. A hook runs the behaviour a script chose, with the parameters a script set, for as many frames as it takes.
...
**Budget.** The VM gets a script budget per frame in the manifest, 1,500 cycles by default, about 15 opcodes at the measured 82 to 118 cycles each. The scheduler runs ready threads round-robin until the budget is spent and resumes where it left off next frame. At 50 frames per second that is about 750 opcodes per second, enough for mission logic and a few dozen entity decisions per second, and by design not enough for anything per frame.
...
The tiers exist so a game can spend where it matters. A racing game on the engine wants physics/sim and ai/basic. Priors-64 probably wants physics/arcade and ai/tactical, because the feel of a crime sandbox is in how the city reacts to you more than in how the car slides. A game can also write its own module against the same header format and hook table, and the picker treats it like any other.
...
**Worked example: Priors-64's vehicle physics.** Priors-64 does not want sim physics. It wants arcade handling with the things that make Coquina Coquina. Its bespoke `vehicle` module depends on physics/arcade and adds:
...
| Media from tile properties | Land vehicles stop at water, boats stop at land, airboats cross both, aircraft ignore tiles and use an altitude byte with a shadow sprite | Reedwater and the Keys depend on it |
| Hydroplaning | Wet tiles after rain lower grip for land vehicles | Ties into the game's weather flag |
| Ramming | Mass ratio decides who stops on impact, without full momentum transfer | The arcade feel at a fraction of sim's cost |
...
| **samples** | 4-bit samples through the SID volume register, streamed from the REU | Minutes of digitised speech | ~25 percent of the CPU while playing (an estimate; and quiet on the 8580 SID, whose volume register carries little DC) |
...
| **quicksave** | Snapshots the 64 KB to the REU and restores it; the 4 KB under the I/O window needs the REU's deferred trigger (command bit 4 clear, I/O banked out, a write to $FF00) | Save anywhere in about four frames (65,536 cycles, 3.3 frames with the screen blanked) | 64 KB DMA |
...
| $5000-$5BFF | 3 KB | engine | Sprite slots: 48, 64 bytes each; a cutscene's colour cells sit above at $5C00 |
| $6000-$7FFF | 8 KB | game | Except $7FFF, the VIC idle byte, which the engine keeps at 0 |
| $8000-$97FF | 6 KB | engine | Overlay region A, managed by the overlay loader |
| $9800-$9FFF | 2 KB | engine | The multiplexer: two halves of a 64-entry list, the groups, the per-position routine table, and the per-sprite interrupt routines generated at start |
...
| $474000 | up to 60 x 8 KB | Code overlay slots (the space to $5FFFFF; `reu.manifest` places what a build actually uses, and the image header at $4FF000 lists it) |
| $600000 | to $7DFFFF | Game heap on an 8 MB REU (`b64_heap_alloc`); the packer never places assets here |
...
The engine ships a Python packer that lays out this image from a manifest, and VICE loads it with `-reuimage`. The image describes itself: a header at $4FF000 carries a magic, the format version, a hash of the slot layout and one descriptor per slot, and every program checks the magic, version and hash at boot with the REU's verify command (`b64_boot_check`). No REU, an empty REU (in VICE, an image whose size differs from the unit) and an image packed from another manifest each halt with their own border colour; `docs/CHECK.md` lists them. On real hardware a boot loader fills the REU from cartridge or disk once per session.
...
| EasyFlash | 1 MB total, so a reduced world | Bank switch plus CPU copy, about 15 cycles per byte for an indexed copy loop | Later, for cartridge release |
| 1541 disk | 170 KB per side | About a second per sector at the stock 300 bytes a second | Development only, never for play |
...
| E1 | **Scroller** | Done, over budget. 8-way scroll over the 4 MB REU world with DMA shifts and double buffering, 50 fps, measured worst frame ~9,200 cycles against a 6,000 target (10,361 for the preparation with traffic in `make check`). `make run-scroll` drives it with the joystick. |
| E2 | **Tilesets** | Region table, tileset streaming, day and night swap behind a ramp. |
| E3 | **Sprites** | Done. Sprite 0 pinned, sprites 1-7 multiplexed over 24 virtual sprites with a y-sorted raster chain, double-buffered lists, and per-slot frame streaming from the REU (a frame is fetched only when a slot's entity changes heading or state). Measured: a static frame with 20 multiplexed cars costs ~1,700 cycles including the chain interrupts. Since 2026-09-18 the list limit follows the CPU speed (24, 32 with the turbo, 64 at 16 MHz and up) and the allocation models the chain's own timing (section 3.4); the turbo tiers are UNTESTED until the hardware runs `tools/b64muxhw.py`. |
| E4 | **Raster** | HUD split done: the playfield is rows 0-22 and row 23 is a fixed HUD row. The split interrupt fires on the last line of row 22 (231 + YSCROLL) and sets YSCROLL to 7 so the HUD's badline always lands on line 239 (when the playfield's YSCROLL is 0 the split line is 231, whose low bits are already 7, so the write must land after cycle 54 of that line or it raises a badline there); the 0-7 idle lines between show background 0 through the idle byte at $7FFF. Forcing a badline mid-row makes the VIC repeat the row, which is why the split has to track the scroll. The multiplexer chain is clamped around the split. Known fault (2026-09-18, found by `examples/traffic`, present in the scroller example too): the split's write lands 60 to 70 cycles after its interrupt, and when that is at cycle 4 or later of the next line the VIC-II has begun the playfield's next row there and the status row shows blank; writes at 234:62 or 235:0-1 show it, 235:4-7 lose it (fine scroll 3). With a fine scroll of 0 or 1 the safe window is a few cycles wide, so the fix needs its own check. Still to do: that fix, the general split table, charset animation, colour cycling. |
...
| E6 | **Overlays** | Loader and tool chain done 2026-09-17: `b64_overlay_load` DMAs an 8 KB slot into region A unless it is already resident (`src/b64_overlay.s`, 60 bytes); overlays assemble for the window with `overlay.cfg` and `tools/b64overlay.py` links them against the resident program's label file so they call engine routines. `examples/overlay` loads two overlays into the same window, calls each through five loads of which four DMA, and proves the last byte of the window arrived; verified at both REU tiers in VICE, 9,013 cycles per 8 KB load. Still to do: region B, persistence records, packages (a screen's code, data and text in one slot), CI. |
...
| E16 | **Baked lighting** | Done, first cut, 2026-09-17 (`examples/showcase`). A lighting state is a background byte plus the screen and colour bytes of the visible rows, 1.6 KB, precomputed by `tools/b64light.py` and applied by `b64_cut_light` with two DMAs in a base-phase vertical blank; the 8 KB bitmap never changes. Modes: a 32-state dusk, and a beacon whose wash falls off with distance from the lamp, one pair of maps per lamp position, so the police strobe lights only the surfaces near the car. Driven from p-code by the LIGHT opcode, or by the object itself: an object file's lights section declares each light's offset, radius, lamp sprite and pattern, `b64light.py object` bakes a state per light, position and colour, and LIGHTS makes the engine draw the lamps and land the maps from the object's position every frame it is drawn, so the light bar and the light it throws cannot drift apart. Limits: the background colour is one register, so pixels drawn in colour 00 cannot be relit per cell; a parked block is not relit (park after lighting, or re-blit). |
...
| E9 | **C128** | Platform layer: MMU setup, far-call trampoline, 2 MHz border switch, resident overlays in bank 1, VDC console. Priors-128 boots in `x128` from the same REU image as Priors-64. |

The forward sequence, phased and with the ideas' sources, is `docs/ROADMAP.md`; this table records what has shipped. Priors-64's Milestone 1 through 5 map onto E1 through E5. Priors-64 begins using the engine at E1. Priors-128 is the same game built for the C128 platform at E9, with its own manifest and no other differences.
...
- Isometric, VSP scrolling (it crashes some machines: Linus Åkesson's "Safe VSP" traces it to DRAM metastability that varies with the board), or anything else that depends on a machine's or a VIC-II revision's quirks. The HUD split's badline placement is ordinary documented behaviour (Bauer, section 3.5).
