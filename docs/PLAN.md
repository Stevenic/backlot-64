...
Budget, measured in the harness (`examples/mux`, `make check`) at its densest layout, a sprite every 4 lines: 32 sprites cost 7,462 cycles in `b64_spr_end` (interrupts off) plus about 3,500 in `b64_spr_add`, and 8,404 cycles of interrupts a frame; the whole list build with its interrupts takes about 19,200 cycles, the entire frame. It holds 50 Hz without a damaged sprite on a stock C64, but leaves nothing for a game, so the platform probe sets the list limit to 24 on a stock machine and 32 when it finds the Ultimate's turbo (`spr_limit`, which a game may raise itself). The previous multiplexer cost 8,172 and 8,891 at 24. Overloaded, 12 sprites bunched into 24 lines, it shows the seven a line can hold and drops the rest whole; the previous one drew some damaged.
