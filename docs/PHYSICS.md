...
| **hull** | boats (speedboat, launch, jet ski) | The car's frame without grip: the sideways speed loses a share of itself each frame (an eighth, a quarter) instead of a fixed amount, so a boat carries its momentum through a turn and slides out wide, throwing spray. Pulling back is reverse thrust, not a brake; with the throttle off, drag takes a thirty-second of the speed a frame. The rudder bites only while water flows past it: half as much when slow, and when still only with thrust held (the propeller's wash). Land is a wall, and so is marsh. An airboat is the same mover with the marsh as water: its fan pushes it over the reeds, where it slides, losing a sixteenth of its sideways speed a frame | measured in `budgets.txt` (`boats.*`, `marsh.*`) |
...
**Marsh.** A water metatile has no roads, so its low property bit is free: set, it is shallow, a marsh (`P_SHALLOW`). A boat's wall mask takes water with that bit clear as its only water; an airboat's (`M_AIRBOAT`, the hull mover under another number) takes any water. The world has a marsh 48 metatiles long south of the beach, from the sand out into the sea. The surface test now asks about water before road bits, since the shallow bit sits among them.
...
| hull (airboat) | airboat | 7 x 7 | 4 | top 4.0, slides a sixteenth; crosses marsh |
...
`make run-marsh` starts on the sand beside the marsh, in the water module, with an airboat on the reeds and a speedboat at sea whose captain (a few lines in the example) holds its throttle west into the marsh. The tape gets into the airboat and runs it south through the reeds, a quarter turn east, and out into the sea past the speedboat, which is pinned against the marsh's edge. It requires no body in its walls, the airboat wholly in the marsh (274 frames) and then clear of it at sea (122), the speedboat against the marsh's edge (240), and two runs alike (`marsh.*`).
