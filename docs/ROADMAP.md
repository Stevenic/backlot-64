...
4. **Instrumentation.** Done 2026-09-17, first cut: the probe (`docs/INSTRUMENT.md`) records events, a timer-driven program-counter sample with the VM thread and script offset, opcode counts and a tick histogram into a block at $F800; `tools/b64probe.py` reads it from VICE or from the C64 Ultimate over REST (hardware path untested) and writes the profile the optimizer will read. Still open: a clock valid under turbo, since the CIA timers' behaviour at 64 MHz is reported both ways and only the hardware will say. *After DOOM C64U.*
