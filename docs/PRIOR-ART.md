...
- **The multiplexer** (after c64gameframework's `UF_SortLoop`, `raster.s` and `sprIrqAdvanceTbl`, and leissa's ninth-sprite handling). Adopted: the persistent sort order, grouped interrupts, running the next group directly when the raster is already past it, and dropping a ninth sprite rather than cutting one. Changed: the order is re-sorted from the last frame's every frame rather than fully sorted only when the count changes; groups are formed from when each previous occupant ends rather than from a fixed five-line window; rejection and grouping are decided in the main loop; and the interrupt code is generated at start, one routine per hardware sprite with its registers built in, into free RAM outside the engine. Measured against this engine's previous multiplexer in the same harness: 5,864 main-loop and 6,428 interrupt cycles for 24 sprites against 8,172 and 8,891, and no damaged sprite when overloaded. Not yet measured against the source; building c64gameframework's example into the same harness is the next step for that comparison.
