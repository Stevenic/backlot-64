...
## Where this engine departs from its sources

The rule: a borrowed idea is credited as is, and any change to it is stated plainly with the reason and, where one exists, the measurement. Improvements are claimed only when measured. Departures so far:

- **Immediate and register forms sharing one handler** (against SCUMM v0's parameter bits in the opcode byte). SCUMM decodes three parameter-mode bits per opcode; this VM loads the immediate into a hidden 33rd variable and jumps into the register handler, so ADDI is ADD with no decode. Measured: 116 cycles for ADD against 147 for the drive body's mixed opcodes; the gain is the absence of decode, at the cost of a larger opcode table (two entries per operation).
- **Slot cache keyed by REU address** (against c64gameframework's depack cache with an age guard). Cadaver depacks compressed sprites into a 48-frame cache; this engine fetches a 64-byte frame by DMA into a slot only when the slot's recorded REU address differs. 169 cycles measured per changed frame, nothing for an unchanged one. Not yet taken from the source: the guard that stops a slot being refilled while it is still being displayed, which this engine needs and lacks.
- **DMA hazards prevented rather than repaired** (against DOOM C64U's save-and-restore of the REU registers inside the music interrupt). DOOM's handler saves `$01` and the REU registers so an interrupt between a transfer's setup and its go cannot corrupt it. This engine masks interrupts around every setup and has the vertical blank save the zero-page scratch it shares with the main loop, so the hazard cannot arise. Different, not measured as better; DOOM's approach is the right one for an interrupt that must issue its own DMA, which this engine's music stream will also need.
- **One image for every tier** (against DOOM C64U's finding that VICE boots an empty REU when the image size differs from the unit). The finding is taken as is; the departure is in tooling: the 16 MB image is the build, the 8 MB image is derived from it by the Makefile, and the boot probe reports which one it got, so there is one artefact and two tiers rather than two builds.
- **The instrumentation** (after DOOM C64U's `instrument.asm` and `clock.asm`). Adopted: a per-frame cycle histogram and program-counter sampling read by a host script. Changed: the sampler runs from a CIA timer at a prime interval rather than at the frame boundary, because a sample taken only at the blank always found the main loop idle; and each sample carries the VM thread and script offset, so one run profiles the p-code as well as the engine. DOOM's finding that the CIA timers keep their 1 MHz clock under turbo is taken as is: an independent measurement on an Ultimate 64 Elite agrees, and this engine's own docs, which had said the opposite, were wrong. Under turbo the probe reports wall time.
- **The audio probe** (after 6510nl's detectaudio). As is, with one change: the module's idle status is required to read zero for 256 consecutive reads before the silent test loop, so an open bus that happens to read a plausible version byte cannot pass. Not measured on hardware yet.
- **The REU image header** (after DOOM C64U's self-describing image with boot cross-checks, `docs/reu-format.md`, `src/mapload.asm`). Adopted: a magic, a format version and a descriptor per block at a fixed place in the image, checked at boot, halting with the reason in the border. Changed: the boot compares seven constant bytes (magic, format, a CRC-16 of the slot names and addresses) with the REU's own verify command instead of fetching the header and cross-checking every descriptor and checksum, because it costs no RAM and about 100 bytes of code, and a layout hash already catches every mismatch between the program and the image. The descriptors stay in the image for tools (`b64pack.py --list`). Not measured against DOOM's loader; the two answer the same question.
- **Colour in the screen code** (after c64gameframework's `drawfullblock` and `drawedge`). Adopted as is: each character's colour is the low four bits of its code, bucketed by the tileset tool. Changed: the source's fills write colour RAM themselves, with per-block flags for one colour or none; here colour RAM is written by one DMA per crossing from the REU copy of the finished screen, which the next shift reads too, so the fills never touch colour and the flags are not needed. Measured on the diagonal crossing: 5,754 cycles for the prepare and 1,119 in the border, against 6,341 and 3,109 before; not measured against the source.
- **The multiplexer** (after c64gameframework's `UF_SortLoop`, `raster.s` and `sprIrqAdvanceTbl`, and leissa's ninth-sprite handling). Adopted: the persistent sort order, grouped interrupts, running the next group directly when the raster is already past it, and dropping a ninth sprite rather than cutting one. Changed: the order is re-sorted from the last frame's every frame rather than fully sorted only when the count changes; groups are formed from when each previous occupant ends, and from when the chain will have finished the group before, rather than from a fixed five-line window and an advance table, with the lead and the lines per entry set by the CPU speed (without the chain in the model, a 64-sprite layout at 1 MHz wrote about 40 entries a frame late; with it none, for about 45 main-loop cycles a sprite, `tools/b64muxhw.py`); rejection and grouping are decided in the main loop; and the interrupt code is generated at start, one routine per hardware sprite with its registers built in, into free RAM outside the engine. Measured against this engine's previous multiplexer in the same harness: 5,864 main-loop and 6,428 interrupt cycles for 24 sprites against 8,172 and 8,891, and no damaged sprite when overloaded. Not yet measured against the source; building c64gameframework's example into the same harness is the next step for that comparison.
