...
### 19 September, later still: the band city, and an image made into a charset

**The asks.** After the helicopter, a long look at how the city should be drawn at all: the five ways of telling a building's height, then GTA-1 against isometric, then ASCII, then a monochrome playfield, which Steven rejected in those words. The direction that survived came from him: "use ai to draw a city scene in black and white. i thin you're on to something by showing that you see building facades on top, then a road, and then roof tops." Three references were generated; all three were liked; the question became "are those really renderable in game?" and then "show me a demo of what gameplay could look like". Shown the old art running, Steven said the clips looked nothing like the references, and set the task: pick a perspective, build a demo that uses the real image, measure it, and answer whether character mode can render everything. Mid-way he asked the question that reframed it: "can we just make this an encoding problem? can we create a design time tool that takes any image and encodes it into a charset representation?" And afterwards, seeing the detail lost: "can we essentially stream them from the REU and dynamically change them on the fly?"

**What was decided.** The variant with everything axis-aligned was chosen over the two with perspective, because only an axis-aligned street tiles. Two paths were built rather than one, so the comparison is a measurement and not an argument: the reference encoded into characters by a tool, and the same street drawn by hand as repeating tiles. The band structure itself — facades along the top, road across the middle, roofs below — replaces the five height cues: elevation is told by which band you are in.

**What was built.** `tools/b64band.py`: scale a picture to C64 pixels, quantise every cell against the shared registers, cluster the cells onto a legal charset with Lloyd's algorithm, and emit an engine tileset — the fitted cells as the charset, every distinct 4x4 block as a metatile, the block ids as a strip the world lays down. Told which rows are road and which pavement, the picture becomes a map: traffic drives on it. `bellamar_band`, the same street drawn by hand in 33 characters. Two districts in the world, at 1200,1200 and 1500,1200, and `-D STREET` and `-D CITYENC` builds of the traffic and scroller examples that visit them. `docs/VIEWS.md` holds the arithmetic.

**What was measured.** Drawn freely the reference wants 852 characters against 192 codes, and 179 of its cells want white against a budget of 12. Encoded it fits in 96, at a mean absolute error of 21 luma of 255 a pixel; as a tileset, 94 characters and 96 metatiles. The hand-drawn set is 33 characters and 22 metatiles. The loss decomposes into roughly equal thirds: 7.0 luma for multicolour's half-width pixels, 14.3 by the time a cell has four colours, 20.9 once the charset is fitted. Doubling the charset buys 1.4; a colour byte a cell buys 2.1; per-band registers with per-band charsets buy 3.4, the largest gain available. Hires buys nothing here — 24 cells of 1536 prefer it. Performance did not move: the same scroller over all three districts finishes its frame by raster line 72-99 of 312 and idles 53-65% of every frame, because the scroller writes one screen code a cell whatever that cell looks like.

**What went wrong.**
- The first encoder was built on the wrong colour rule: it let a multicolour cell take dark or light grey as its own colour. A cell's colour is the low three bits of its nibble, so only the first eight are reachable — which `docs/ART.md` has said since it was written. The map viewer drew the roads cyan and the roofs purple, which is exactly what the chip would have done. Corrected in the tool and the tileset; the first round of measurements was discarded.
- The day set stopped packing. Three metatiles added for the three-quarter district's ids pushed two colour values past their twelve codes. The day set only needs the ids to line up, so it now points them at art it already has.
- Two REU slots overlapped: a tileset is 12 KB and the flat set had been given 4 KB of room.
- Every program flashed an orange border. The manifest had changed, so the layout hash no longer matched and the boot check halted — the check doing its job, and the demos needing a rebuild.
- `make check` failed on `debris.frames_lost`, 81 against 77. Running the same tape against both versions of the physics module showed the cause: a body with altitude no longer sleeps, because one that did froze in mid-air. The fix is right, so the budget moved to 85 with both measurements written beside it.
...
| The image encoder let a multicolour cell take grey as its own colour | The map viewer, drawing the roads cyan | The rule was already in `docs/ART.md`; the tool and the tileset now follow it |
| Debris froze in mid-air, and fixing it cost five frames in 560 | `make check`, on `debris.frames_lost` | The budget moved to 85, with both measurements beside it |
