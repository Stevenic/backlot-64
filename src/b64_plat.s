...
;
; After Gideon Zweijtzer, Ultimate documentation (turbo mode settings,
; Ultimate Audio Register API v0.2, Command Interface).  As is.
; Audio probe after 6510nl, ModPlayer_16k detectaudio (read in xahmol's
; UltimateDemo2026 audio.c).  Changed: the idle status must read zero for
; 256 reads before the test loop, so open bus cannot pass.  See CREDITS.md.
; Register facts:
